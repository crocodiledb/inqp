...
case class CountDS(start: String, end: String, count: Int)
...
object StructuredNetworkWordCountWindowed {
  ...
  def main(args: Array[String]) {
    ...
    // Group the data by window and word and compute the count of each group
     val windowedCounts = words.withWatermark("timestamp", "1 seconds")
       .groupBy(window($"timestamp", windowDuration, slideDuration), $"word").count()
      // .map(row => CountDS(row.getAs[String](0), row.getAs[String](1), row.getAs[Int](2)))
      // .toDF()
    ...
    val query = windowedCounts.writeStream
      ...
      .start()
      // .option("path", "hdfs://localhost:9000/spark_data")
      // .option("checkpointLocation", "hdfs://localhost:9000/spark_checkpoint")
      // .start()
