...
public final class SlothAggResultMap {
  ...
  public KVIterator<UnsafeRow, UnsafeRow> iterator() {
    return new KVIterator<UnsafeRow, UnsafeRow>() {
      ...
      private boolean materializeKey = false;
      private boolean materializeValue = false;
      private BytesToBytesMap.Location loc = null;

      @Override
      public boolean next() {
        if (mapLocationIterator.hasNext()) {
          loc = mapLocationIterator.next();
          materializeKey = false;
