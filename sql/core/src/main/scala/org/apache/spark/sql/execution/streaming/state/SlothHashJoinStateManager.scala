...
class SlothHashJoinStateManager (
      ...
      override def getNext(): KeyWithIndexAndValueWithCounter = {
        ...
        while (allKeyToNumValues.hasNext) {
          ...
          if (removalCondition(currentKey)) {
            keyWithIndexToValue.
              removeAllValues(currentKey, currentKeyToNumValue.numValue)
          } else if (isScan) {
  ...
  private class KeyWithIndexToValueStore extends StateStoreHandler(KeyWithIndexToValueType) {
    ...
    protected val stateStore = getStateStore(keyWithIndexSchema, valueWithCounterSchema)

    private val hashMap = HashMap.empty[UnsafeRow, ValueWithCounter]
    ...
    def get(key: UnsafeRow, valueIndex: Long): ValueWithCounter = {
      ...
      if (optionValue.isDefined) optionValue.get
        ...
        if (rawValueWithCounter == null) null
          ...
          hashMap.put(keyWithIndex, valueWithCounter)
          valueWithCounter
        }
      }
    }
